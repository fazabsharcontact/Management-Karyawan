<x-app-layout>
    ...
    <div class="p-6">
        <div class="bg-white rounded-lg shadow p-6 max-w-lg mx-auto">
            <form method="POST" action="{{ route('admin.master-potongan.store') }}" class="space-y-4">
                @csrf
                <div>
                    <label for="nama_potongan" class="block font-medium text-sm text-gray-700">Nama Potongan</label>
                    <input type="text" name="nama_potongan" id="nama_potongan" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block mt-1 w-full" value="{{ old('nama_potongan') }}" required autofocus>
                </div>

                {{-- INPUT BARU DITAMBAHKAN --}}
                <div>
                    <label for="jumlah_default" class="block font-medium text-sm text-gray-700">Jumlah Default (Rp)</label>
                    <input type="number" name="jumlah_default" id="jumlah_default" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block mt-1 w-full" value="{{ old('jumlah_default') }}" placeholder="Contoh: 50000">
                </div>
