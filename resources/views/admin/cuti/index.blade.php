            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                @if (session('success'))
                    <div class="mb-6 p-4 rounded-md bg-green-50 text-green-700 border border-green-200 flex items-start">
                        <svg class="w-5 h-5 mr-3 mt-1 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                            fill="currentColor">
                ...
                @if (session('error'))
                    <div class="mb-6 p-4 rounded-md bg-red-50 text-red-700 border border-red-200 flex items-start">
                        <svg class="w-5 h-5 mr-3 mt-1 flex-shrink-0" xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="mb-8 p-6 rounded-lg bg-gray-50 border border-gray-200">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <div class="text-right">
                            <p id="realtime-clock" class="text-2xl font-mono font-bold text-gray-700">
                                {{ $now->format('H:i:s') }}</p>
                    ...
                    @if ($isWeekend)
                        <div class="text-center p-6 bg-blue-50 border border-blue-200 rounded-lg">
                            ...
                            <p class="text-sm text-blue-700 mt-2">Tidak ada presensi yang perlu dilakukan hari ini.
                                Nikmati waktu istirahat Anda.</p>
                        </div>
                    @elseif (!$absensiHariIni)
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div
                                class="p-4 bg-blue-50 rounded-lg border border-blue-200 text-center flex flex-col justify-between">
                                ...
                                <form action="{{ route('pegawai.kehadiran.store') }}" method="POST">
                                    ...
                                    <button type="submit"
                                        class="w-full px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 font-semibold shadow-sm transition duration-150">PRESENSI
                                        MASUK</button>
                                </form>
                            </div>
                            <div
                                class="p-4 bg-indigo-50 rounded-lg border border-indigo-200 text-center flex flex-col justify-between">
                                ...
                                <button onclick="openIzinModal('Izin')"
                                    class="w-full px-4 py-2 bg-indigo-500 text-white rounded-md hover:bg-indigo-600 font-semibold shadow-sm transition duration-150">Input
                                    Izin</button>
                            </div>
                            <div
                                class="p-4 bg-pink-50 rounded-lg border border-pink-200 text-center flex flex-col justify-between">
                                ...
                                <button onclick="openIzinModal('Sakit')"
                                    class="w-full px-4 py-2 bg-pink-500 text-white rounded-md hover:bg-pink-600 font-semibold shadow-sm transition duration-150">Input
                                    Sakit</button>
                            </div>
                        </div>
                    @else
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div
                                class="p-4 rounded-lg flex items-center @if ($absensiHariIni->status == 'Hadir') bg-green-50 border border-green-200 @elseif($absensiHariIni->status == 'Terlambat') bg-yellow-50 border border-yellow-200 @else bg-gray-100 border border-gray-200 @endif">
                                <div class="mr-4">
                                    <div
                                        class="flex items-center justify-center h-12 w-12 rounded-full @if ($absensiHariIni->status == 'Hadir') bg-green-100 text-green-600 @elseif($absensiHariIni->status == 'Terlambat') bg-yellow-100 text-yellow-600 @else bg-gray-200 text-gray-600 @endif">
                                ...
                                <div>
                                    <h4
                                        class="font-semibold @if ($absensiHariIni->status == 'Hadir') text-green-800 @elseif($absensiHariIni->status == 'Terlambat') text-yellow-800 @else text-gray-800 @endif">
                                        Presensi Masuk</h4>
                                    @if (in_array($absensiHariIni->status, ['Hadir', 'Terlambat']))
                                        <p class="text-2xl font-bold">
                                            {{ \Carbon\Carbon::parse($absensiHariIni->jam_masuk)->format('H:i') }}</p>
                            ...
                            <div class="p-4 rounded-lg flex items-center bg-gray-100 border border-gray-200">
                                <div class="mr-4">
                                    <div
                                        class="flex items-center justify-center h-12 w-12 rounded-full @if ($absensiHariIni->jam_pulang) bg-green-100 text-green-600 @else bg-gray-200 text-gray-600 @endif">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M17 16l4-4m0 0l-4-4m4 4H7m5 4v1a3 3 0 003 3h4a3 3 0 003-3V7a3 3 0 00-3-3H12a3 3 0 00-3 3v1" />
                                        </svg>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    @elseif(in_array($absensiHariIni->status, ['Hadir', 'Terlambat']))
                                        <form action="{{ route('pegawai.kehadiran.store') }}" method="POST"
                                            ...
                                            @csrf
                                            <button type="submit"
                                                class="w-full px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 font-semibold shadow-sm transition duration-150">PRESENSI
                                                PULANG</button>
                                        </form>
                                    @else
                            ...
                            <div class="mt-2 px-7 py-3">
                                <form action="{{ route('pegawai.kehadiran.store') }}" method="POST"
                                    ...
                                    <div class="mb-4 text-left">
                                        <label for="bukti" class="block text-sm font-medium text-gray-700">Upload
                                    ...
                                    <div class="mb-4 text-left">
                                        ...
                                        <textarea name="keterangan" id="keterangan" rows="3"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" placeholder="Tulis alasan Anda..."></textarea>
                                    </div>
                                    <div class="items-center px-4 py-3">
                                        <button type="submit" id="modal-submit-btn"
                                            class="px-4 py-2 w-full bg-indigo-600 text-white text-base font-medium rounded-md shadow-sm hover:bg-indigo-700"></button>
                                    </div>
                                </form>
                            </div>
                            <div class="text-center px-4 py-2"><button id="cancel-izin-modal-button"
                                    class="text-sm text-gray-500 hover:text-gray-700">Batal</button></div>
                        </div>
                    </div>
                </div>

                {{-- Modal Konfirmasi Pulang Awal --}}
                <div id="early-clockout-modal"
                    class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
                    <div class="relative top-20 mx-auto p-5 border w-full max-w-lg shadow-lg rounded-md bg-white">
                        <div class="mt-3">
                            <div class="flex items-start">
                                <div
                                    class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-yellow-100 sm:mx-0">
                                ...
                                <div class="ml-4 text-left">
                                    ...
                                    <div class="mt-2">
                                        <p class="text-sm text-gray-500">Anda akan melakukan presensi pulang sebelum
                                            jam 17:00. Aksi ini akan tercatat dan dapat mempengaruhi rekapitulasi
                                            kehadiran atau perhitungan gaji. Apakah Anda yakin?</p>
                                    </div>
                                </div>
                            </div>
                            <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                                <button type="button" id="confirm-early-button"
                                    class="inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 bg-yellow-500 text-white text-base font-medium hover:bg-yellow-600 sm:ml-3 sm:w-auto sm:text-sm">Ya,
                                    Tetap Pulang</button>
                                <button type="button" id="cancel-early-button"
                                    class="mt-3 inline-flex justify-center w-full rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:w-auto sm:text-sm">Batal</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-8">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <form method="GET" class="flex flex-wrap gap-3">
                            <select name="bulan"
                                class="border-gray-300 rounded-md shadow-sm w-full md:w-auto text-sm">
                            ...
                            <button type="submit"
                                class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-md shadow text-sm">Tampilkan</button>
                        </form>
                    </div>
                                ...
                                @forelse($kehadiran as $row)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            {{ \Carbon\Carbon::parse($row->tanggal)->isoFormat('dddd, D MMMM Y') }}
                                        </td>
                                        <td class="px-4 py-3 text-center">
                                            <span
                                                class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full
                                                 @if ($row->status == 'Cuti') bg-purple-100 text-purple-800 
                                                 @elseif($row->status == 'Hadir') bg-green-100 text-green-800 
    ...
    @push('scripts')
        <script>
            function openIzinModal(status) {
                ...
                document.getElementById('modal-text').innerText = 'Anda akan mengajukan status ' + status +
                    ' hari ini. Mohon sertakan keterangan dan bukti jika ada.';
