<x-app-layout>
    <div class="p-6 space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-gray-700 mb-4">Filter & Pencarian</h3>

            <form method="GET" action="{{ route('admin.tugas.index') }}"
                class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 text-sm">
                <div class="lg:col-span-2">
                    <label for="search" class="block font-medium text-gray-700">Cari Judul / Pegawai</label>
                    <input type="search" name="search" id="search" value="{{ request('search') }}"
                        placeholder="Ketik untuk mencari..." class="mt-1 w-full border-gray-300 rounded-md shadow-sm">
                </div>
                ...
                <div>
                    <label for="jabatan_id" class="block font-medium text-gray-700">Jabatan</label>
                    <select name="jabatan_id" id="jabatan_id" class="mt-1 w-full border-gray-300 rounded-md shadow-sm">
                        <option value="">Semua</option>
                        @foreach ($jabatans as $item)
                            <option value="{{ $item->id }}" {{ request('jabatan_id') == $item->id ? 'selected' : '' }}>
                                {{ $item->nama_jabatan }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="divisi_id" id="divisi_id" class="mt-1 w-full border-gray-300 rounded-md shadow-sm">
                        ...
                        @foreach ($divisis as $item)
                            <option value="{{ $item->id }}" {{ request('divisi_id') == $item->id ? 'selected' : '' }}>
                                {{ $item->nama_divisi }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex gap-2 items-end">
                    ...
                    <a href="{{ route('admin.tugas.index') }}"
                        class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-medium py-2 px-4 rounded-md shadow w-full text-center">
                        Reset
                    </a>
                </div>
            </form>
            ...
            <div class="overflow-x-auto">
                <table class="w-full border border-gray-200 rounded">
                    <thead class="bg-gray-100">
                        <tr>
                            <th class="border-b px-4 py-2 text-left text-sm font-medium text-gray-600">Judul Tugas</th>
                            <th class="border-b px-4 py-2 text-left text-sm font-medium text-gray-600">Penerima</th>
                            <th class="border-b px-4 py-2 text-left text-sm font-medium text-gray-600">Pemberi</th>
                            <th class="border-b px-4 py-2 text-left text-sm font-medium text-gray-600">Tenggat</th>
                            <th class="border-b px-4 py-2 text-center text-sm font-medium text-gray-600">Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($tugas as $t)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="border-b px-4 py-2 text-sm text-gray-600">
                                    {{ \Carbon\Carbon::parse($t->tenggat_waktu)->format('d M Y, H:i') }}
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-6 text-gray-500">
                                    Tidak ada data tugas yang cocok dengan kriteria.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div id="add-task-modal"
        class="fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full hidden z-50 transition-opacity duration-300">
        <div class="relative top-10 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
            <div class="flex justify-between items-center border-b pb-3">
                <h3 class="text-xl font-semibold text-gray-900">Form Tambah Tugas Baru</h3>
                <button id="close-modal-button"
                    class="text-gray-400 hover:text-gray-600 text-3xl font-light">&times;</button>
            </div>
            <div class="mt-5">
                <form action="{{ route('admin.tugas.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label for="judul_tugas_modal" class="block text-sm font-medium text-gray-700">Judul
                            Tugas</label>
                        <input type="text" id="judul_tugas_modal" name="judul_tugas"
                    ...
                    <div>
                        ...
                        <textarea id="deskripsi_modal" name="deskripsi" class="mt-1 w-full border-gray-300 rounded-md shadow-sm"
                            rows="3"></textarea>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="search-pegawai-input"
                                class="block text-sm font-medium text-gray-700">Penerima
                                Tugas</label>
                            <div class="relative mt-1">
                                <input type="text" id="search-pegawai-input"
                                ...
                                <div id="pegawai-dropdown-list"
                                    class="absolute z-10 w-full bg-white border border-gray-300 rounded-md mt-1 max-h-40 overflow-y-auto hidden shadow-lg">
                                    @foreach ($pegawais as $p)
                                        <div class="cursor-pointer p-2 hover:bg-gray-100 pegawai-item"
                                            data-id="{{ $p->id }}" data-nama="{{ $p->nama }}">
                                            {{ $p->nama }}
                                        </div>
                                    @endforeach
                                    <div id="no-pegawai-found"
                        ...
                        <div>
                            <label for="tenggat_waktu_modal"
                                class="block text-sm font-medium text-gray-700">Tenggat
                                Waktu</label>
                            <input type="datetime-local" id="tenggat_waktu_modal" name="tenggat_waktu"
                                class="mt-1 w-full border-gray-300 rounded-md shadow-sm" required>
                        </div>
                    </div>
