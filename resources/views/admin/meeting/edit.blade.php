<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">✏️ Edit Meeting: {{ $meeting->judul }}</h2>
    </x-slot>

    <div class="p-6">
        <div class="bg-white rounded-lg shadow p-6 max-w-2xl mx-auto">
            <form method="POST" action="{{ route('admin.meeting.update', $meeting->id) }}" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                    <label for="judul" class="block font-medium text-sm text-gray-700">Judul Meeting</label>
                    <input type="text" name="judul" id="judul" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block mt-1 w-full" value="{{ old('judul', $meeting->judul) }}" required>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="waktu_mulai" class="block font-medium text-sm text-gray-700">Waktu Mulai</label>
                        <input type="datetime-local" name="waktu_mulai" id="waktu_mulai" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block mt-1 w-full" value="{{ old('waktu_mulai', \Carbon\Carbon::parse($meeting->waktu_mulai)->format('Y-m-d\TH:i')) }}" required>
                    </div>
                    <div>
                        <label for="waktu_selesai" class="block font-medium text-sm text-gray-700">Waktu Selesai</label>
                        <input type="datetime-local" name="waktu_selesai" id="waktu_selesai" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block mt-1 w-full" value="{{ old('waktu_selesai', \Carbon\Carbon::parse($meeting->waktu_selesai)->format('Y-m-d\TH:i')) }}" required>
                    </div>
                </div>
                 <div>
                    <label for="lokasi" class="block font-medium text-sm text-gray-700">Lokasi</label>
                    <input type="text" name="lokasi" id="lokasi" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block mt-1 w-full" value="{{ old('lokasi', $meeting->lokasi) }}" required>
                </div>
                ...
                <div>
                    ...
                    <select name="pembuat_id" id="pembuat_id" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block mt-1 w-full" required>
                        ...
                        @foreach($pegawais as $pegawai)
                            <option value="{{ $pegawai->id }}" {{ old('pembuat_id', $meeting->pembuat_id) == $pegawai->id ? 'selected' : '' }}>
                                {{ $pegawai->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="deskripsi" id="deskripsi" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block mt-1 w-full h-24">{{ old('deskripsi', $meeting->deskripsi) }}</textarea>
                </div>
                
                {{-- --- PERUBAHAN UTAMA: DARI SELECT MULTIPLE MENJADI CHECKBOX DENGAN SEARCH --- --}}
                <div>
                    ...
                    <div id="peserta-list" class="mt-2 border border-gray-300 rounded-md max-h-60 overflow-y-auto">
                        ...
                        <label class="peserta-item flex items-center space-x-3 p-2 border-b last:border-b-0 hover:bg-gray-50">
                            <input type="checkbox" name="peserta_ids[]" value="{{ $pegawai->id }}" 
                                ...
                                {{-- Menambahkan 'checked' jika ID pegawai ada di dalam array $pesertaIds --}}
                                {{ in_array($pegawai->id, $pesertaIds) ? 'checked' : '' }}>
                ...
                <div class="flex items-center gap-4">
                    <button type="submit" class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-md shadow">Update Jadwal</button>
